...
## Table of Contents
- [Overview](#overview)
- [HTTP API (`goose-server`)](#http-api-goose-server)
  - [OpenAPI Specification](#openapi-specification)
  - [Authentication](#authentication)
  - [Key Endpoints (Summary)](#key-endpoints-summary)
- [FFI (Foreign Function Interface) APIs](#ffi-foreign-function-interface-apis)
  - [1. `goose-ffi`](#1-goose-ffi)
  - [2. `goose-llm`](#2-goose-llm)
- [MCP (Model Context Protocol)](#mcp-model-context-protocol)

## Overview
This document provides a reference for the various APIs exposed by `codename goose`. This includes the main HTTP API for agent interaction, Foreign Function Interfaces (FFI) for library usage from other languages, and mentions of the Model Context Protocol (MCP) used for tool extensions.

## HTTP API (`goose-server`)
The `goose-server` component (binary `goosed`) exposes an HTTP API for controlling the agent, managing sessions, configuration, and more. This is the primary API used by the `ui/desktop` application.

### OpenAPI Specification
The API is formally defined by an OpenAPI (Swagger) specification.

**Location of the OpenAPI schema file in the repository:** `ui/desktop/openapi.json`

**To generate the schema (as per `Justfile` and `crates/goose-server/Cargo.toml`):**
```bash
cargo run -p goose-server --bin generate_schema
```
This command will output the JSON schema, which is typically saved to `ui/desktop/openapi.json`.

The following is a pretty-printed JSON representation of the OpenAPI schema:
```json
{
  "openapi": "3.0.3",
  "info": {
...
### Authentication
- Some endpoints, particularly under `/config/`, do not seem to require explicit authentication from the OpenAPI schema.
- Endpoints like `/context/manage` and `/sessions/{session_id}` show a `security` section with `api_key`, but the definition of this `api_key` (e.g., header name, how it''s obtained) is not detailed in the provided schema structure itself. It''s implied to be a bearer token or a custom header, managed by the server''s `AuthProvider`. Standard practice would be an `Authorization` header.
- The actual secret key for server access is likely configured during the server setup and managed internally.

### Key Endpoints (Summary)
Based on the paths in the OpenAPI schema:
- **Agent Interaction**:
    - `/agent/tools`: Get available tools.
    - `/confirm` (for `confirm_permission`): Send user replies/confirmations to the agent.
    - `/context/manage`: Manage the context of a session (truncate, summarize).
- **Configuration Management (`/config/*`)**:
    - `/config`: Read all configuration.
    - `/config/backup`: Backup configuration.
    - `/config/init`: Initialize/check configuration.
    - `/config/upsert`, `/config/remove`, `/config/read`: Manage specific config keys.
    - `/config/extensions`: List, add, remove extensions.
    - `/config/providers`: List available LLM providers.
    - `/config/permissions`: Manage tool permissions.
- **Session Management (`/sessions/*`)**:
    - `/sessions`: List all active/available sessions.
    - `/sessions/{session_id}`: Get history for a specific session.
- **Scheduling (`/schedule/*`)**:
    - `/schedule/create`, `/schedule/list`, `/schedule/delete/{id}`, `/schedule/{id}` (update): Manage scheduled tasks.
    - `/schedule/{id}/pause`, `/schedule/{id}/unpause`, `/schedule/{id}/run_now`, `/schedule/{id}/kill`, `/schedule/{id}/inspect`: Control and inspect scheduled jobs.
    - `/schedule/{id}/sessions`: Get sessions related to a schedule.

For detailed request/response formats, refer to the full OpenAPI specification embedded above.

## FFI (Foreign Function Interface) APIs

### 1. `goose-ffi`
- **Purpose**: Provides C-compatible bindings for the core Goose AI agent framework.
- **Documentation**: Refer to `crates/goose-ffi/README.md`.
- **Key Features**:
    - Create and manage Goose agents.
    - Configure providers (initially Databricks, extensible).
    - Send messages to agents and receive responses.
- **Building**: `cargo build --package goose-ffi` (generates dynamic library and `include/goose_ffi.h`).
- **Usage**: See Python example in `crates/goose-ffi/examples/goose_agent.py`.

### 2. `goose-llm`
- **Purpose**: Provides FFI for stateless LLM provider interactions, prompt utilities, summarization, and truncation.
- **Documentation**: Refer to `crates/goose-llm/README.md`.
- **Key Features**:
    - Chat completion with model providers.
    - Read-only tool detection.
    - Summarization/truncation methods.
- **Building & Binding Generation**:
  ```bash
  # Build Rust library
  cargo build -p goose-llm
  # Generate Kotlin bindings (example)
  ...
  ```
- **Usage**: See examples in `bindings/kotlin/example/Usage.kt` and `bindings/python/usage.py`.

## MCP (Model Context Protocol)
- **Purpose**: MCP is used for implementing tool extensions as separate processes or services.
- **Documentation**: While not a direct "API" in the same sense as HTTP or FFI, the protocol is defined in `mcp-core`.
    - `crates/mcp-client/README.md` and `crates/mcp-server/README.md` provide context on usage.
- **Interaction**: The `goose` agent uses an MCP client to communicate with MCP servers (e.g., those provided by `goose-mcp` or custom ones).
- **Testing**: MCP servers can be tested with the `@modelcontextprotocol/inspector` tool.
  Example from `crates/goose-mcp/README.md`:
  ```bash
  npx @modelcontextprotocol/inspector cargo run -p goose-mcp --example mcp
  ```

This section serves as a pointer to MCP rather than a detailed spec, as MCP itself is a protocol enabling tool APIs.
